...
for riga in df_lista_ridotta.itertuples():
    ...
    if not df1.empty:
        ...
        donne_dictionary[id_donna]["tweets"]["numero_tweets_pre_classifica"] = df1.shape[0]
        ...
        for tweet in df1.itertuples():
            if tweet not in donne_dictionary[id_donna]["tweets"]["pre_classifica"]:
                donne_dictionary[id_donna]["tweets"]["pre_classifica"].append(tweet)
    else:
    ...
    if not df2.empty:
        ...
        donne_dictionary[id_donna]["tweets"]["numero_tweets_post_classifica"] = df2.shape[0]
        df2.set_index('id', inplace=True)
        df2 = df2[['date', 'tweet', 'language', 'hashtags', 'user_id', 'username', 'name', 'nlikes', 'nreplies', 'nretweets']]

        for tweet in df2.itertuples():
            if tweet not in donne_dictionary[id_donna]["tweets"]["post_classifica"]:
                donne_dictionary[id_donna]["tweets"]["post_classifica"].append(tweet)
    else:
