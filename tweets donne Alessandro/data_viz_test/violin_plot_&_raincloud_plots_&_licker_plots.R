...
# le tre categorie sono state modificate dall'interno dell'excel, lo stesso
# si può fare 
likert_db <- read_excel("1_prima_viz_MODIFICATA.xlsx")
# likert_db <- as.data.frame(likert_db)
# likert_db <- likert_db[, 5:10]

#######################################
# dovresti provare a creare un nuovo dataframe
# con due colonne:
# 1. Domanda
# 2. Valore
# e fillarlo tramite la replica di 180 valori
# rispetto alle domande e andando ad incollare
# per colonna le varie colonne dei dati.
# Altrimenti farlo in excel

#######################################

likert_db$Domanda <- as.factor(likert_db$Domanda)
likert_db$Valore <- as.factor(likert_db$Valore)

# Conversione in dataframe altrimenti dà errore sul numero diverso di categorie
likert_db <- as.data.frame(likert_db)

# usa la funzione likert
lik_items <- likert(items=likert_db[,2, drop = FALSE], grouping=likert_db[,1])

# plotta il grafico
plot(lik_items)

# se vuoi cambiare l'ordine delle categorie ricorda:
# funzionano esattamente al contrario. Se "Valore complessivo" lo vuoi
# visulizzato per ultimo deve nell'excel essere inserito come primo elemento
...
likert_db <- read_excel("2_seconda_viz_MODIFICATA.xlsx")
...
likert_db <- read_excel("3_terza_viz_MODIFICATA.xlsx")
...
likert_db <- read_excel("4_quarta_viz_MODIFICATA.xlsx")
...
likert_db <- read_excel("5_quinta_viz_MODIFICATO.xlsx")
