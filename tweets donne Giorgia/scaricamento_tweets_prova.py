...
# lista donne bbc
df_lista_donne = pd.read_csv("tweets donne Giorgia/tabella_finale_prova.csv", sep=',')
...
for riga in df_lista_ridotta.itertuples():
    ...
    if riga.year == 2015:
        ...
        until2 = datetime(2015, 11, 30, 00, 00, 00)
        ...
        until1 = until1 + timedelta(days=1)  # per considerare tutte le 24 ore del primo giorno
        since1 = datetime(2019, 10, 1, 00, 00, 00)
        changing1 = until1 - timedelta(days=1)
        remaining_days1 = int(str(changing1 - since1).split()[0])

        # seconda finestra
        until2 = datetime(2019, 10, 30, 00, 00, 00)
        until2 = until2 + timedelta(days=1)  # per considerare tutte le 24 ore del primo giorno
    ...
    document = {
        'id': riga.id,
        'nome': riga.name,
        "numero_tweets_pre_classifica": 0,
        "numero_tweets_post_classifica": 0,
        'tweets': {
            "pre_classifica": [],
            "post_classifica": []
        }
    ...
    if not df1.empty:
        ...
        document["numero_tweets_pre_classifica"] = df1.shape[0]
        ...
        for tweet in df1.itertuples():
            if tweet not in document["tweets"]["pre_classifica"]:
                document["tweets"]["pre_classifica"].append(tweet)
    else:
    ...
    if not df2.empty:
        ...
        document["numero_tweets_post_classifica"] = df2.shape[0]
        df2.set_index('id', inplace=True)
        df2 = df2[['date', 'tweet', 'language', 'hashtags', 'user_id', 'username', 'name', 'nlikes', 'nreplies', 'nretweets']]

        for tweet in df2.itertuples():
            if tweet not in document["tweets"]["post_classifica"]:
                document["tweets"]["post_classifica"].append(tweet)
    else:
...
with open('results_prova_2.json', 'w', encoding="UTF-8") as f:
